...
f2_status_t _libbmp_decompress_bitfields( libbmp_bmp_reader_state * state )
{
	...
	expected_image_offset = offset +
		3*sizeof(uint32_t);										// 3 color masks
...
f2_status_t _libbmp_compress_bitfields( libbmp_bmp_writer_state * state )
{
	...
	debugbreak_if( 16 != pf->bits_per_pixel && 24 != pf->bits_per_pixel )
		return F2_STATUS_ERROR_INVALID_STATE;	// only 24 bit images are supported
